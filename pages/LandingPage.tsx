  ...
  return (
    <div className="min-h-screen font-['Outfit'] scroll-smooth">
      ...
      <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/80 backdrop-blur-xl border-b border-slate-200/50 py-4 shadow-sm' : 'bg-transparent py-6'}`}>
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-3">
            <div className="w-10 h-10 bg-black rounded-xl flex items-center justify-center text-white shadow-lg shadow-slate-200 ring-2 ring-slate-100">
              <Zap fill="currentColor" size={20} />
            </div>
            <span className="text-2xl font-extrabold text-slate-900 tracking-tight">XGo</span>
          </Link>

          <div className="hidden md:flex items-center gap-8 text-sm font-bold text-slate-500">
            <button onClick={() => document.getElementById('home')?.scrollIntoView({ behavior: 'smooth' })} className="hover:text-black transition-colors">Home</button>
            ...
            <button onClick={() => document.getElementById('how-it-works')?.scrollIntoView({ behavior: 'smooth' })} className="hover:text-black transition-colors">How it Works</button>
            <button onClick={() => document.getElementById('approach')?.scrollIntoView({ behavior: 'smooth' })} className="hover:text-black transition-colors">Live Demo</button>
            <button onClick={() => document.getElementById('testimonials')?.scrollIntoView({ behavior: 'smooth' })} className="hover:text-black transition-colors">Reviews</button>
          ...
          <div className="flex items-center gap-4">
            ...
            <Link to="/signup" className="hidden sm:flex px-6 py-3 bg-slate-900 text-white rounded-2xl font-bold border border-slate-800 hover:bg-black hover:border-black transition-all shadow-xl hover:shadow-slate-200 active:scale-95 items-center gap-2">
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-white border-b border-slate-200 p-6 space-y-4 animate-in slide-in-from-top-4 duration-300 shadow-xl">
            <div className="flex flex-col gap-4 text-base font-bold text-slate-600">
              <button onClick={() => { setIsMobileMenuOpen(false); document.getElementById('home')?.scrollIntoView({ behavior: 'smooth' }); }} className="text-left py-2 hover:text-black border-b border-slate-50 transition-colors">Home</button>
              <button onClick={() => { setIsMobileMenuOpen(false); document.getElementById('features')?.scrollIntoView({ behavior: 'smooth' }); }} className="text-left py-2 hover:text-black border-b border-slate-50 transition-colors">Features</button>
              <button onClick={() => { setIsMobileMenuOpen(false); document.getElementById('how-it-works')?.scrollIntoView({ behavior: 'smooth' }); }} className="text-left py-2 hover:text-black border-b border-slate-50 transition-colors">How it Works</button>
              <button onClick={() => { setIsMobileMenuOpen(false); document.getElementById('approach')?.scrollIntoView({ behavior: 'smooth' }); }} className="text-left py-2 hover:text-black border-b border-slate-50 transition-colors">Live Demo</button>
              <button onClick={() => { setIsMobileMenuOpen(false); document.getElementById('testimonials')?.scrollIntoView({ behavior: 'smooth' }); }} className="text-left py-2 hover:text-black border-b border-slate-50 transition-colors">Reviews</button>
              <button onClick={() => { setIsMobileMenuOpen(false); document.getElementById('pricing')?.scrollIntoView({ behavior: 'smooth' }); }} className="text-left py-2 hover:text-black border-b border-slate-50 transition-colors">Pricing</button>
            </div>
            ...
            <div className="flex flex-col gap-3 pt-4">
              ...
              <Link to="/signup" onClick={() => setIsMobileMenuOpen(false)} className="w-full py-4 text-center bg-black text-white font-black rounded-2xl shadow-lg shadow-slate-200 active:scale-95 transition-all">
                Get Started Free
              </Link>
            </div>
          </div>
      ...
      <section id="home" className="pt-32 pb-32 px-6 relative overflow-hidden">
        ...
        <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-slate-300/40 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/2 -z-10 animate-pulse-slow"></div>
        ...
        <div className="max-w-7xl mx-auto text-center space-y-10 relative z-10">
          ...
          <div className="inline-flex items-center gap-2 bg-white/80 backdrop-blur-md border border-slate-200/60 rounded-full pl-2 pr-4 py-1.5 text-sm font-bold text-slate-600 shadow-sm shadow-slate-200 hover:scale-105 transition-transform cursor-default animate-fade-in-up">
            <span className="bg-black text-white text-[10px] px-2 py-0.5 rounded-full uppercase tracking-wider font-extrabold">New</span>
          ...
          <h1 className="text-6xl md:text-8xl font-extrabold text-slate-900 tracking-tight leading-[0.95] max-w-5xl mx-auto animate-fade-in-up delay-100">
            Dominate <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#FF4500] to-[#FF8700] relative">
              ...
              <svg className="absolute w-full h-3 -bottom-1 left-0 text-slate-200 -z-10" viewBox="0 0 100 10" preserveAspectRatio="none">
          ...
          <p className="text-lg md:text-xl text-slate-500 font-medium max-w-2xl mx-auto leading-relaxed animate-fade-in-up delay-200">
            The all-in-one growth engine. Identify high-intent conversations, generate value-first AI replies, and turn Authority into customers.
          </p>
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 md:gap-8 text-slate-600 font-bold mt-6 animate-fade-in-up delay-200">
            <div className="flex items-center gap-2.5 bg-white/50 backdrop-blur-sm px-4 py-2 rounded-full border border-slate-200/50 shadow-sm">
              <MessageSquare size={20} className="text-black" fill="currentColor" fillOpacity={0.2} />
              <span>Smart Comments</span>
            </div>
            <div className="flex items-center gap-2.5 bg-white/50 backdrop-blur-sm px-4 py-2 rounded-full border border-slate-200/50 shadow-sm">
              <PenTool size={20} className="text-black" fill="currentColor" fillOpacity={0.2} />
              <span>Auto Posts</span>
            </div>
            <div className="flex items-center gap-2.5 bg-white/50 backdrop-blur-sm px-4 py-2 rounded-full border border-slate-200/50 shadow-sm">
              <Bot size={20} className="text-black" fill="currentColor" fillOpacity={0.2} />
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-6 animate-fade-in-up delay-300">
            <Link to="/signup" className="w-full sm:w-auto px-10 py-5 bg-gradient-to-r from-black to-slate-800 text-white rounded-[2rem] font-bold text-xl hover:shadow-2xl hover:shadow-slate-500/40 hover:-translate-y-1 transition-all flex items-center justify-center gap-3">
          ...
          <div className="pt-24 relative animate-fade-in-up delay-500">
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] bg-gradient-to-b from-slate-200/20 to-transparent rounded-full blur-3xl -z-10 pointer-events-none"></div>
      ...
      <section id="features" className="py-24 bg-[#f8fafc] relative overflow-hidden">
        ...
        <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-slate-200/40 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"></div>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="text-center max-w-3xl mx-auto mb-20 space-y-4">
            ...
            <p className="text-slate-500 text-lg font-medium">We've automated the tedious parts of X marketing so you can focus on building relationships.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              },
              {
                ...
                desc: 'Track Authority, engagement, and click-through rates in real-time.',
      ...
      <section id="how-it-works" className="py-24 bg-white relative">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
            {/* Step 1 */}
            <div className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-lg hover:shadow-2xl hover:shadow-slate-200/50 hover:-translate-y-2 transition-all duration-300 group relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="w-16 h-16 bg-white border-2 border-slate-100 text-black rounded-2xl flex items-center justify-center text-2xl font-bold mb-8 shadow-sm group-hover:scale-110 transition-transform">01</div>
                <h3 className="text-2xl font-bold text-slate-900 mb-3">Connect & Train</h3>
                <p className="text-slate-500 leading-relaxed font-medium">Link your X account and define your brand voice. The AI learns your unique style in seconds.</p>
              </div>
            </div>
            ...
            <div className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-lg hover:shadow-2xl hover:shadow-green-100/50 hover:-translate-y-2 transition-all duration-300 group relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <p className="text-slate-500 leading-relaxed font-medium">Track traffic, leads, and Authority growth. Scale your presence effortlessly while you sleep.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-24 bg-slate-50 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 relative">
            ...
            <div className="hidden md:block absolute top-16 left-[12%] right-[12%] h-0.5 bg-gradient-to-r from-slate-200 via-slate-200 to-slate-200 z-0"></div>

            {[
              {
                icon: Search,
                title: "Scans Your Niche",
                desc: "Identifies relevant subXs and tracks keyword mentions in real-time.",
                textColor: 'text-black',
                cornerBg: 'bg-slate-50',
                borderColor: 'border-slate-100',
                hoverShadow: 'hover:shadow-slate-200/50'
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="space-y-8 relative z-10">
              <span className="text-black font-bold tracking-widest text-sm uppercase">Live Demo</span>
              <h2 className="text-4xl md:text-5xl font-extrabold text-slate-900 leading-tight">
                Smart enough to sound <span className="text-black">human.</span>
              </h2>
              ...
              <div className="space-y-6">
                {[
                  ...
                  <div key={i} className="flex gap-4">
                    <div className="flex-shrink-0 w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center text-black mt-1">
            ...
            <div className="relative">
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] bg-gradient-to-br from-slate-100/50 to-blue-100/50 rounded-full blur-3xl -z-10"></div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white p-8 rounded-[2rem] shadow-xl border border-slate-200 relative z-10 transform transition-transform hover:scale-[1.02] duration-300">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center text-black font-bold border border-slate-200">
                      <span className="text-xs">u/</span>
                    </div>
                    <div>
                      <div className="text-sm font-bold text-slate-900">u/tech_founder_99</div>
                      <div className="text-xs text-slate-400">posted in #SaaS • 2h ago</div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-4 text-slate-400 text-sm font-bold border-t border-slate-100 pt-4">
                    <span className="flex items-center gap-1 hover:text-black transition-colors"><MessageSquare size={16} /> 24 Comments</span>
                    <span className="flex items-center gap-1 hover:text-green-500 transition-colors"><TrendingUp size={16} /> 95% Upvoted</span>
                  </div>
                </div>

                {/* Connection Line */}
                <div className="h-10 w-0.5 bg-gradient-to-b from-slate-200 to-slate-300 mx-auto"></div>

                {/* AI Reply Card - Solution */}
                <div className="bg-slate-900 p-6 rounded-[2rem] shadow-2xl border border-slate-800 relative overflow-hidden group hover:border-black/50 transition-colors duration-500">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-slate-500/10 rounded-full blur-2xl -mr-10 -mt-10 group-hover:bg-slate-500/20 transition-colors duration-500"></div>
                  ...
                  <p className="text-slate-300 leading-relaxed mb-5 text-sm font-medium">
                    Cold email is tough in 2026. Instead of outbound, have you tried <span className="text-white bg-slate-500/20 px-1 py-0.5 rounded border border-black/20">engaging in existing discussions</span>? We built a tool that automatically finds these threads...
                  </p>

                  <div className="flex gap-3">
                    <button className="flex-1 bg-black hover:bg-slate-800 text-white py-2.5 rounded-xl text-xs font-bold transition-all shadow-lg shadow-black/20 hover:shadow-slate-500/30 transform hover:-translate-y-0.5 flex items-center justify-center gap-2">
      ...
      <section id="testimonials" className="py-24 relative overflow-hidden bg-slate-50/50">
        <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
          ...
          <div className="relative min-h-[450px] flex items-center justify-center">
            {[
              { name: 'Sarah Jenkins', role: 'CTO, TechFlow', text: "I was skeptical about X marketing, but this tool made it feel authentic. We got our first 100 users in a week.", image: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&auto=format&fit=crop&q=60" },
              ...
              { name: 'David Chen', role: 'Founder, SaaSI', text: "Finally, a tool that respects X's culture while helping businesses grow. Absolute game changer for us.", image: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&auto=format&fit=crop&q=60" }
            ].map((t, index) => (
              <div
                key={index}
              ...
              >
                <div className="bg-white p-12 md:p-16 rounded-[3rem] shadow-2xl shadow-slate-200 border border-slate-100 max-w-3xl mx-auto relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <div className="flex gap-1 text-yellow-400 mb-8 justify-center">
                    ...
                    <div className="flex items-center justify-center gap-4">
                      <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-slate-100 shadow-md">
                        <img src={t.image} alt={t.name} className="w-full h-full object-cover" />
                      </div>
                      <div className="text-left">
                        <div className="font-bold text-slate-900 text-lg">{t.name}</div>
                        <div className="text-black font-medium text-sm">{t.role}</div>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <section id="pricing" className="py-24 bg-gradient-to-b from-slate-50 to-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16 space-y-6">
            <span className="bg-slate-200 text-black px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest inline-flex items-center gap-2">
              <Star size={12} fill="currentColor" /> Simple Pricing
            </span>
            <h2 className="text-4xl md:text-5xl font-extrabold text-slate-900 tracking-tight">
              Pay as you <span className="text-black">grow.</span>
            </h2>
            ...
            <div className="flex items-center justify-center gap-4 mt-12 bg-white/50 backdrop-blur-sm border border-slate-200/50 p-2 rounded-3xl w-fit mx-auto shadow-sm">
              ...
              <div className="relative">
                <button
                  ...
                  className={`px-6 py-2 rounded-2xl text-sm font-bold transition-all flex items-center gap-2 ${billingCycle === 'yearly' ? 'bg-black text-white shadow-lg' : 'text-slate-500 hover:text-slate-700'}`}
                >
                  Yearly
                </button>
                {plans.length > 0 && (() => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto items-stretch">
            {plans.filter((p: any) => p.isVisible !== false).map((plan: any) => {
              const theme = plan.isPopular ? 'dark' : (plan.name === 'Agency' ? 'slate' : 'slate');
              ...
              return (
                <div key={plan.id} className={`bg-white rounded-[2.5rem] p-10 border ${plan.isPopular ? 'border-slate-200 shadow-2xl shadow-slate-200/50 scale-105 z-10' : 'border-slate-100 shadow-lg'} hover:-translate-y-2 transition-all duration-500 relative overflow-hidden flex flex-col ${plan.purchaseEnabled === false ? 'opacity-80 grayscale-[0.2]' : ''}`}>
                  {plan.isPopular && (
                    <div className="absolute top-0 right-0 w-48 h-48 bg-slate-50 rounded-bl-[160px] -mr-12 -mt-12 pointer-events-none transition-transform hover:scale-110"></div>
                  )}

                  {plan.isPopular && (
                    <div className="absolute top-6 right-6 bg-slate-200 text-black px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest shadow-sm">
                      Most Popular
                    </div>
                  )}
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    <div className="mb-8">
                      <div className="flex flex-col gap-2">
                        {!isFree && plan.purchaseEnabled === false && (
                          <span className="bg-black text-white px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest w-fit mb-1 shadow-sm shadow-slate-200">
                            Reached Capacity
                          </span>
                        )}
                        <span className={`font-black uppercase tracking-widest text-[10px] px-3 py-1 rounded-full w-fit ${plan.isPopular ? 'bg-slate-50 text-black' : 'bg-slate-50 text-slate-500'}`}>{plan.name}</span>
                    ...
                    <ul className="space-y-4 mb-10 flex-1">
                      <li className="flex items-center gap-3.5 text-sm font-black text-slate-900 border-b border-slate-50 pb-2">
                        <div className={`w-6 h-6 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm ${plan.isPopular ? 'bg-slate-200 text-black' : 'bg-slate-100 text-slate-600'}`}>
                      ...
                      <li className="flex items-center gap-3.5 text-sm font-medium text-slate-700">
                        <div className={`w-6 h-6 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm ${plan.maxAccounts > 1 ? (plan.isPopular ? 'bg-slate-200 text-black' : 'bg-slate-100 text-slate-600') : 'bg-slate-50 text-slate-300'}`}>
                      ...
                      <li className="flex items-center gap-3.5 text-sm font-medium text-slate-700">
                        <div className={`w-6 h-6 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm ${plan.allowImages ? (plan.isPopular ? 'bg-slate-200 text-black' : 'bg-slate-100 text-slate-600') : 'bg-slate-50 text-slate-300'}`}>
                      ...
                      <li className="flex items-center gap-3.5 text-sm font-medium text-slate-700">
                        <div className={`w-6 h-6 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm ${plan.allowTracking ? (plan.isPopular ? 'bg-slate-200 text-black' : 'bg-slate-100 text-slate-600') : 'bg-slate-50 text-slate-300'}`}>
                      ...
                      {plan.features.map((f: string, i: number) => (
                        <li key={i} className="flex items-center gap-3.5 text-sm font-medium text-slate-500">
                          <div className={`w-6 h-6 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm ${plan.isPopular ? 'bg-slate-200 text-black' : 'bg-slate-50 text-slate-400'}`}>
                    ...
                    <Link
                      ...
                      className={`w-full py-5 rounded-[2rem] font-black text-lg transition-all shadow-xl hover:shadow-2xl hover:-translate-y-1 text-center block ${plan.purchaseEnabled === false
                        ...
                        : plan.isPopular
                          ? 'bg-gradient-to-r from-black to-slate-800 text-white shadow-slate-200'
      ...
      <section className="py-24 bg-white relative overflow-hidden">
        <div className="max-w-6xl mx-auto px-6">
          <div className="bg-black rounded-[3rem] p-12 md:p-20 text-center relative overflow-hidden shadow-2xl shadow-slate-200">
            {/* Background Pattern */}
            ...
            <div className="relative z-10 max-w-3xl mx-auto space-y-8">
              ...
              <p className="text-slate-100 text-lg font-medium max-w-2xl mx-auto leading-relaxed">
                Set up once. Your agent comments, posts, and grows your presence every single day.
              </p>

              <div className="flex flex-col items-center pt-4">
                <Link to="/signup" className="px-10 py-5 bg-white text-black rounded-full font-bold text-lg hover:bg-slate-50 hover:scale-105 transition-all shadow-xl shadow-black/10 mb-6">
                  Activate Your AI Agent
                </Link>
      ...
      <footer className="py-12 bg-white text-slate-500">
        <div className="max-w-7xl mx-auto px-6 flex flex-col items-center justify-center gap-6">
          <div className="text-sm font-medium">
            © 2026 XGo AI. All rights reserved.
          </div>
