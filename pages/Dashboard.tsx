...
export const Dashboard: React.FC = () => {
  ...
  const fetchData = useCallback(async (isInitial = false) => {
    ...
    try {
      ...
      const [histRes, profileRes, XRes] = await Promise.allSettled([
        fetch(`/api/user/replies/sync?userId=${user.id}`),
        fetch(`/api/user/x/profile?userId=${user.id}`),
  ...
  const totalLikes = history.reduce((a, b) => a + (b.ups || 0), 0);
  const activetopics = new Set(history.map(r => r.topic)).size;

  // X Followers as "reach" if connected, otherwise Likes-based estimate
  const totalReach = profile?.totalFollowers ?? (totalLikes > 0 ? totalLikes * 18 : 0);

  // Week-over-week trends
  const RepliesTrend = fmtTrend(pctChange(thisWeek.length, lastWeek.length));
  const LikesTrend = fmtTrend(
    pctChange(
  ...
  const subsTrend = fmtTrend(
    pctChange(
      new Set(thisWeek.map(r => r.topic)).size,
      new Set(lastWeek.map(r => r.topic)).size,
    ),
  );
  ...
  const byDate: Record<string, { Replies: number; Likes: number }> = {};
  history.forEach(r => {
    const key = toDateKey(new Date(r.deployedAt));
    if (!byDate[key]) byDate[key] = { Replies: 0, Likes: 0 };
    byDate[key].Replies += 1;
    byDate[key].Likes += r.ups || 0;
  });

  const chartData = days7.map(dateKey => {
    const d = new Date(dateKey);
    const label = d.toLocaleDateString('en-US', { weekday: 'short' });
    const entry = byDate[dateKey] || { Replies: 0, Likes: 0 };
    return {
      day: label,
      date: dateKey,
      Replies: entry.Replies,
      Likes: entry.Likes,
      reach: entry.Likes * 18,
    };
  });
    ...
    <div className="max-w-7xl mx-auto space-y-10 animate-fade-in font-['Outfit'] pt-4">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-slate-400 font-medium text-sm pl-4">Your X growth at a glance.</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {/* X connection badge */}
          <div className={`flex items-center gap-2 px-4 py-2 rounded-2xl border text-xs font-bold ${XConnected ? 'bg-green-50 border-green-200 text-green-700' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>
            <span className={`w-2 h-2 rounded-full ${XConnected ? 'bg-green-500 animate-pulse' : 'bg-slate-300'}`} />
            {XConnected ? `X: u/${profile?.name || 'Connected'}` : 'X: Not linked'}
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <StatCard
          ...
          sub={profile?.totalFollowers != null ? 'Total followers (live)' : 'Based on Likes'}
          icon={Users}
          accent="bg-purple-600"
          trend={profile?.totalFollowers != null ? 'Live' : undefined}
          isLoading={isLoading}
        />
        <StatCard
          label="Topics"
          value={activetopics}
          sub={`${new Set(thisWeek.map(r => r.topic)).size} active this week`}
          icon={Globe}
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-xs text-slate-400 font-medium mt-0.5">
                Real Likes &amp; Replies — last 7 days
              </p>
            </div>
            <div className="flex items-center gap-4 text-xs font-bold text-slate-400">
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded-full bg-black inline-block" />Likes
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded-full bg-blue-500 inline-block" />Replies
              </span>
            </div>
          </div>
            ...
            <div className="h-[260px]">
              <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={100}>
                <AreaChart data={chartData}>
                  ...
                  />
                  <Area type="monotone" dataKey="Likes" name="Likes" stroke="#000000" strokeWidth={3} fill="url(#gUp)" />
                  <Area type="monotone" dataKey="Replies" name="Replies" stroke="#3b82f6" strokeWidth={3} fill="url(#gCom)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          )}
            ...
            <div className="h-[260px]">
              <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={100}>
                <BarChart data={chartData} barSize={14}>
                  ...
                  />
                  <Bar dataKey="reach" name="Reach" fill="#000000" radius={[8, 8, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-200/60 shadow-sm overflow-hidden">
            ...
            {isLoading ? (
              <div className="flex items-center justify-center p-16">
                <RefreshCw className="animate-spin text-black" size={28} />
              </div>
            ) : recent.length === 0 ? (
              <div className="flex flex-col items-center justify-center p-16 gap-4 text-center">
                ...
                <p className="text-slate-400 text-xs">
                  Go to{' '}
                  <Link to="/comment-agent" className="text-black font-bold hover:underline">Replies</Link>
                ...
                {recent.map((row) => (
                  <div key={row.id} className="flex items-center gap-4 px-7 py-5 hover:bg-slate-50/60 transition-colors group">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-slate-900 text-sm truncate group-hover:text-black transition-colors">
                        {row.postTitle}
                      </p>
                    ...
                    <div className="flex items-center gap-3 text-slate-500 shrink-0">
                      ...
                      <div className="flex items-center gap-1.5">
                        <Heart size={13} className={row.ups > 0 ? 'text-green-500' : 'text-slate-300'} />
                    ...
                    <a href={row.postUrl} target="_blank" rel="noreferrer" className="opacity-0 group-hover:opacity-100 transition-opacity">
                      <ExternalLink size={14} className="text-slate-400 hover:text-black" />
                    </a>
                  </div>
                ))}
              </div>
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-6">
            <div className="space-y-4">
              ...
              <QuickAction
                icon={PenTool}
                title="Post Agent"
                desc="Create full X posts with AI"
                to="/post-agent"
                accent="bg-blue-600"
              />
            </div>
          ...
          {/* X Followers card — only shown when connected */}
          {profile && (
            <div className="bg-slate-900 rounded-[1.75rem] p-6 text-white relative overflow-hidden">
              <div className="absolute top-0 right-0 w-24 h-24 bg-black/20 rounded-full blur-2xl -mr-6 -mt-6" />
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">X Account</p>
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 bg-black rounded-xl flex items-center justify-center font-black text-sm">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white/10 rounded-xl p-3 text-center">
                  <p className="text-lg font-black">{(profile.commentFollowers ?? 0).toLocaleString()}</p>
                  <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">Comment Followers</p>
                </div>
                <div className="bg-white/10 rounded-xl p-3 text-center">
                  <p className="text-lg font-black">{(profile.linkFollowers ?? 0).toLocaleString()}</p>
                  <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">Link Followers</p>
                </div>
              </div>
              {profile.totalFollowers != null && (
